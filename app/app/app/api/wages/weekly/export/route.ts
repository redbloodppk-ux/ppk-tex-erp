...
export async function GET(req: Request): Promise<Response> {
  ...
  const weeklyColumns: ExcelColumn[] = [
    ...
    { key: 'reallocated_in', label: 'Reallocated in', type: 'rupee', width: 14, total: true },
    { key: 'reallocated_out', label: 'Reallocated out', type: 'rupee', width: 15, total: true },
  ...
  const weeklyRows: Array<Record<string, unknown>> = data.per_employee.map((p) => {
    ...
    return {
      ...
      reallocated_in: p.reallocated_in,
      reallocated_out: p.reallocated_out,
