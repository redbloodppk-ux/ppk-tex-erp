...
import { Boxes, Package, Layers, AlertTriangle, Coins, TrendingDown } from 'lucide-react';
...
export default async function WarehousePage({
  ...
  const tab = (sp.tab ?? 'yarn') as 'yarn' | 'bobbin' | 'fabric';
  ...
  // ─── Low-stock alerts (cross-cutting, always visible) ─────────────────────
  const lowStock = await computeLowStock(supabase, counts ?? [], bobbinMasters ?? []);

  // ─── Tab-specific data ────────────────────────────────────────────────────
  const yarnRows   = tab === 'yarn'   ? await loadYarn(supabase, sp, mills ?? [], counts ?? []) : null;
  const bobbinRows = tab === 'bobbin' ? await loadBobbin(supabase, sp, mills ?? [], customers ?? [], bobbinMasters ?? []) : null;
  const fabricRows = tab === 'fabric' ? await loadFabric(supabase, sp) : null;

  return (
    <div>
      <PageHeader
        title="Warehouse — Unified Stock"
        subtitle="Yarn, bobbins and fabric on one screen. Low-stock alerts highlighted above. Adjustments + delivery challan coming in Phase 2-3."
      />
      ...
      <form className="card p-3 mb-4 flex flex-wrap items-end gap-3" method="GET">
        ...
        {(sp.mill || sp.customer || sp.count || sp.location) && (
          <Link href={`/app/warehouse?tab=${tab}`} className="btn-sm bg-cloud text-ink-soft hover:bg-cloud/80">
            Clear
          </Link>
        )}
      </form>
      ...
      {/* ── Tab body ───────────────────────────────────────────────────── */}
      {tab === 'yarn'   && <YarnView rows={yarnRows!} />}
      {tab === 'bobbin' && <BobbinView rows={bobbinRows!} />}
      {tab === 'fabric' && <FabricView rows={fabricRows!} />}
    </div>
  );
}
...
async function loadBobbin(
  supabase: any, sp: SP, mills: any[], customers: any[], bobbins: any[],
): Promise<BobbinRow[]> {
  let q = supabase
    .from('bobbin_stock')
...
async function loadFabric(supabase: any, _sp: SP): Promise<FabricRow[]> {
