...
export default function NewCostingPage() {
  ...
  async function handleSaveSubmit() {
    ...
    const payload = {
      ...
      // (and the Fabric Costing list page that depends on it) can
      // show Quoted ₹/m and True ₹/m without recomputing. True =
      // quoted until LOOMS overhead per metre is wired in.
...
function LatestRateHint({
  countId, currentRate, latest, apply,
}: {
  countId: string;
  currentRate: number;
  ...
  return (
    <div className="mt-1 flex flex-wrap items-center gap-1.5 text-[11px] text-ink-mute">
      ...
      <button
        ...
        className="inline-flex items-center rounded-full bg-indigo-600 text-white text-[10px] font-semibold px-2 py-0.5 hover:bg-indigo-700"
      >
