...
export default async function JobworkPartiesPage({
  ...
  return (
    <div>
      ...
      {/* Mobile / PWA: card view. The jobwork parties table is wide and
          forces horizontal scrolling on a phone, so below md we render each
          party as a tap-friendly card. The table is hidden on mobile. */}
      <CardFilter placeholder="Search jobwork parties…">
        {rows.length ? rows.map((c) => (
          <div key={c.id} className="card p-3">
            <div className="flex items-start justify-between gap-2">
              <div className="min-w-0">
                <Link href={`/app/jobwork-parties/${c.id}`} className="font-semibold text-ink hover:text-indigo break-words">
                  {c.name}
                </Link>
