...
const GROUPS: CorrGroup[] = [
  ...
  },
  {
    ...
    cards: [
      ...
      { code: 'CORR-A6', title: 'Dashboard attendance widgets', status: 'done',
        evidence: 'migration 027 (v_today_attendance_widget) + app/components/dashboard/today-attendance.tsx',
        blurb: 'Today\u2019s Attendance tile on the Operations dashboard. Surfaces Morning + Night present counts vs target so the owner sees missing crew at a glance.' },
  ...
  },
  {
    ...
    cards: [
      { code: 'CORR-H1', title: 'PWA install flow polish', status: 'done',
        evidence: 'public/manifest.json + icon-{192,512}.png + apple-touch-icon + app/components/install-prompt.tsx (mounted in AppShell) + offline-banner + offline-sync + update-prompt',
        blurb: 'Icons, splash, manifest, beforeinstallprompt-driven Install button (iOS Safari hint included), offline banner, sync queue + update-available banner.' },
      ...
      { code: 'CORR-H3', title: 'Audit log viewer', status: 'done',
        evidence: 'app/audit/page.tsx (list + filters + pagination) + app/audit/[id]/page.tsx (before/after JSON diff)',
        blurb: 'Filterable list (table / action / actor / date / row id), 50/page pagination. Detail page renders side-by-side before/after diff with unchanged fields collapsible.' },
      { code: 'CORR-H4', title: 'Rate limiting on writes', status: 'done',
        evidence: 'lib/rate-limit.ts (token bucket, unit tested) + lib/supabase/middleware.ts (POST/PUT/PATCH/DELETE gated, returns 429 + Retry-After)',
        blurb: 'Per-user (auth uid) / per-IP fallback token bucket. Env-tunable: RATE_LIMIT_WRITES_PER_MIN (default 120) and RATE_LIMIT_BURST (default 30). GET stays uncapped.' },
      { code: 'CORR-H5', title: 'PDF templates for invoice / DC', status: 'done',
        blurb: "Server-rendered PDFs matching the user's existing paperwork. Hand-built by owner outside this tracker." },
      { code: 'CORR-H6', title: 'Notification fan-out', status: 'done',
        evidence: 'lib/notifications/source.ts + /api/notifications + /api/notifications/count + app/components/notification-bell.tsx (in topbar) + app/notifications/page.tsx (full list with severity + kind filters)',
        blurb: 'Server-derived notifications (no extra table): pending costing approvals + critical low-stock. Polling bell with severity-coloured badge + dropdown + full list. Overdue invoices + variance flags deferred to v1.1.' },
      { code: 'CORR-H7', title: 'Backup runbook + restore drill', status: 'done',
        evidence: 'docs/RESTORE.md',
        blurb: 'PITR enablement, weekly off-site export design, restore procedures (PITR / daily / off-site), verification checklist, quarterly drill SOP.' },
      { code: 'CORR-H8', title: 'Rollback runbook', status: 'done',
        evidence: 'docs/ROLLBACK.md',
        blurb: 'Reversible-migration pairing, 60-day shakedown undo policy, per-entity rollback procedures (invoice / receipt / payment / costing).' },
