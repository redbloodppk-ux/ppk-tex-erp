...
export function JobworkBillForm({ parties }: JobworkBillFormProps): React.ReactElement {
  ...
  // or the GST % changes.
  //
  // Bills with GST filled in run on a separate numbering series (JWB for
  // jobwork_invoice, WGB for weaving_bill) from bills without GST (JB /
  // WB) — see migration 249_jobwork_weaving_gst_prefix.sql. The trigger
  // decides this from the real invoice.gst_amount at save time; here we
  // mirror that using the GST % the operator has typed so far (gst_amount
  // is 0 whenever GST % is 0, and >0 whenever GST % is >0 and there's a
  // taxable amount).
  useEffect(() => {
    let cancelled = false;
    void (async (): Promise<void> => {
      ...
      const gstFilled = num(gstPct) > 0;
      const seqKey = gstFilled ? `${docType}_gst` : docType;
  ...
  async function handleSave(): Promise<void> {
    ...
    // (its own running series per prefix, reset each financial year), so
    // the bill date must be on/after the latest already-issued bill in the
    // same series. JB/WB (no GST) and JWB/WGB (GST-filled) are independent
    // series that share the same invoice.doc_type (see migration
    // 249_jobwork_weaving_gst_prefix.sql), so we additionally scope this
    // check by invoice_no prefix — otherwise a date gap between the two
    // series would wrongly block a valid save. ISO date strings
    // (YYYY-MM-DD) compare as plain strings.
    {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const sbAny = supabase as any;
      const gstFilled = round2(totals.cgst + totals.sgst + totals.igst) > 0;
      const seqKey = gstFilled ? `${docType}_gst` : docType;
      const { data: seqRow } = await sbAny
        ...
        .eq('doc_type', seqKey)
        .maybeSingle();
      const prefix = (seqRow as { prefix?: string } | null)?.prefix ?? '';
      let lastBillQuery = sbAny
        .from('invoice')
        .select('invoice_no, invoice_date')
        .eq('doc_type', docType)
        .not('invoice_date', 'is', null);
      if (prefix !== '') lastBillQuery = lastBillQuery.like('invoice_no', `${prefix}/%`);
      const { data: lastBill } = await lastBillQuery
