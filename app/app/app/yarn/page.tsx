...
export const metadata = { title: 'Yarn & Mills' };

export default async function YarnPage() {
  const supabase = await createClient();
  const [{ data: lots }, { data: cover }] = await Promise.all([
    supabase.from('yarn_lot').select('id, lot_no, yarn_count_code, mill_name, qty_kg, rate_per_kg, received_date').order('received_date', { ascending: false }).limit(30),
    supabase.from('v_yarn_days_of_cover').select('yarn_count_code, on_hand_kg, days_of_cover').order('days_of_cover'),
  ]);

  return (
    <div>
      <PageHeader title="Yarn & Mills" subtitle="Yarn purchase lots, mill scorecards and days-of-cover by count." />
            ...
            <tbody>
              {lots.map((l: any) => (
                <tr key={l.id} className="border-t border-line/40 hover:bg-haze/60">
                  <td className="px-4 py-3 font-mono text-xs">{l.lot_no}</td>
                  <td className="px-4 py-3 font-semibold">{l.yarn_count_code}</td>
                  <td className="px-4 py-3">{l.mill_name}</td>
                  <td className="px-4 py-3 text-right num">{formatKg(l.qty_kg)}</td>
                  <td className="px-4 py-3 text-right num">{formatRupee(l.rate_per_kg, { decimals: 2 })}</td>
