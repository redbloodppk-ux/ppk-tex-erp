  ...
  if (soIds.length > 0) {
    ...
    if (linkedDcIds.length > 0) {
      ...
      const { data: items } = await sb.from('delivery_challan_item')
        .select('dc_id, metres, fabric_quality:fabric_quality_id ( meter_per_pc, fabric_type )').in('dc_id', dcIdSet);
      const soByDc = new Map(linkedDcIds.map((d) => [d.id, d.sales_order_id]));
      for (const it of (items ?? []) as Array<{ dc_id: number; metres: number | string | null; fabric_quality: { meter_per_pc: number | string | null; fabric_type: string | null } | null }>) {
        const soId = soByDc.get(it.dc_id);
        if (soId == null) continue;
        // Towels (fabric_type = 'towel') store the towel COUNT in `metres`, not
        // real metres. Convert to real metres (count × meter_per_pc) so the
        // pcs↔m ratio below reconverts it correctly. Fabric / woven / dhoties
        // also carry meter_per_pc, so we MUST gate on fabric_type — not
        // meter_per_pc > 0 — otherwise real metres get wrongly multiplied.
        const isTowel = it.fabric_quality?.fabric_type === 'towel';
        const mpp = Number(it.fabric_quality?.meter_per_pc ?? 0);
        const realM = isTowel && mpp > 0 ? Number(it.metres ?? 0) * mpp : Number(it.metres ?? 0);
