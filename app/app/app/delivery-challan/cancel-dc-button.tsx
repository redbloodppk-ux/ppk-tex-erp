...
export function CancelDcButton({
  ...
  async function openDialog() {
    ...
    // Only in-house DCs consume production-batch stock, so only they need a
    // target-batch picker. Jobwork / outsource cancel straight away.
    if (productionMode !== 'inhouse') return;

    setLoading(true);
    try {
      // 1. The DC's items — which batch fed them, and what quality.
      const { data: itemRows } = await sb
        .from('delivery_challan_item')
        .select('production_batch_id, fabric_quality_id')
        .eq('dc_id', dcId);
      ...
      // Not cut from a batch → no stock to relocate, skip the picker.
