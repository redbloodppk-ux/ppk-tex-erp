-- 152_production_vs_delivery_filter_inhouse.sql
--
-- Bug fix on migration 151. The in-house attribution CTE was pulling
-- shift_log metres for any loom that had a fabric_quality_id set,
-- regardless of that quality's production_mode. So a loom physically
-- weaving a jobwork-tagged quality ended up reporting its shift
-- metres under the In-house row in the Production vs Delivery report.
--
-- The fix: require fq.production_mode = 'inhouse' on the in-house
-- attribution. Shift logs against looms assigned to non-inhouse
-- qualities are dropped from the in-house mode. Jobwork / outsource
-- production already flows in via the fabric_receipt branch, which is
-- the canonical "production complete" event for those modes.
...
LANGUAGE sql STABLE SECURITY INVOKER AS $func$
  ...
  ih_attributed AS (
    ...
    JOIN public.fabric_quality fq
      ON fq.id = l.fabric_quality_id
     AND fq.production_mode = 'inhouse'
    GROUP BY fq.id
  ),
  ih_unattributed AS (
