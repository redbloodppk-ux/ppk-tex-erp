-- 215_so_refresh_status_respect_closed.sql
--
-- Make fn_so_refresh_status honour the manual 'closed' state added in
-- migration 214. A closed order must stay closed even if a later DC or
-- invoice change fires the auto-status trigger, so 'closed' is added to
-- the early-return guard alongside cancelled / draft / pending_approval.
--
-- (Function body is otherwise identical to migration 213 — it keeps the
-- towel pcs-count to real-metres conversion for the delivered total.)
