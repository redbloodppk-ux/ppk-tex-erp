-- 213_so_status_towel_delivered_fix.sql
--
-- Fixes the Sales Order auto-status "delivered" calculation for TOWEL
-- deliveries.
--
-- Background: for towel qualities (fabric_quality.meter_per_pc > 0) the
-- delivery_challan_item.metres column actually holds the towel PIECE
-- COUNT, not real running metres. fn_so_refresh_status (migration 194)
-- summed dci.metres directly and compared it against ordered metres,
-- so an 800-towel delivery against a 1275 m order looked like 800 m
-- (63 %) and the SO was wrongly stamped 'partial_dispatch' instead of
-- 'dispatched'.
--
-- Fix: when summing delivered metres, convert towel rows back to real
-- metres (count × meter_per_pc). Plain fabric rows (meter_per_pc 0/NULL)
-- keep metres as-is. Everything else in the function is unchanged.
...
BEGIN
  ...
  -- Towel rows store the piece COUNT in `metres`; convert to real metres
  -- (count × meter_per_pc) so the % comparison below is apples-to-apples.
  SELECT COALESCE(SUM(
    CASE
      WHEN COALESCE(fq.meter_per_pc, 0) > 0 THEN dci.metres * fq.meter_per_pc
      ELSE dci.metres
    END
  ),0) INTO v_delivered
