-- 155_production_vs_delivery_dci_metres_is_pcs.sql
--
-- Operators have been entering piece-priced DC items (towels / dhoties
-- with fabric_quality.meter_per_pc set) by typing the PIECE COUNT into
-- the dci.metres column. The "metres" column on those rows therefore
-- holds pieces, not metres. Verified against the linked fabric
-- receipts: e.g. DC#61 for FQ-0001 has dci.metres=2,545 and the
-- matching receipt is 4,835.50 m = 2,545 * 1.90.
--
-- Convention applied here (Production vs Delivery report only):
--
--   if fq.meter_per_pc IS NOT NULL AND > 0
--     delivered_pcs = dci.metres            -- the typed value IS pcs
--     delivered_m   = dci.metres * meter_per_pc
--   else
--     delivered_m   = dci.metres            -- typed value IS metres
--     delivered_pcs = NULL
--
-- Produced side is unchanged: shift_log net metres is always actual
-- metres (typed per-piece in the shift form and summed up).
--
-- Stock ledger / warehouse pages and the DC creation form are NOT
-- touched by this migration — this only changes how the report reads
-- DC items.
...
LANGUAGE sql STABLE SECURITY INVOKER AS $func$
  ...
  -- Convert dci.metres to actual metres at the source. Piece-priced
  -- qualities multiply by meter_per_pc; running-fabric qualities pass
  -- the value through unchanged.
  delivered AS (
    SELECT dci.fabric_quality_id, dc.production_mode::text AS production_mode,
           SUM(
             CASE
               WHEN fq.meter_per_pc IS NOT NULL AND fq.meter_per_pc > 0
