-- 153_production_vs_delivery_label_by_fq_mode.sql
--
-- Migration 152 dropped shift_log metres for looms tagged with
-- non-inhouse qualities to avoid a perceived double-count with
-- fabric_receipt. In practice the two sources are distinct:
--
--   shift_log         = cloth physically woven on the mill's own
--                       looms (regardless of whose yarn / design)
--   fabric_receipt    = cloth received back from an external weaver
--
-- So both paths are real production and they should be additive.
-- The mode each path lands on:
--
--   shift_log         → fabric_quality.production_mode of the loom's
--                       assigned quality. The fabric_production_mode
--                       enum uses 'job_work' (with underscore) so we
--                       cast to text and normalise to 'jobwork' to
--                       line up with the DC/receipt branch.
--   fabric_receipt    → delivery_challan.production_mode (already
--                       'jobwork' / 'outsource' in the DC enum).
...
LANGUAGE sql STABLE SECURITY INVOKER AS $func$
  ...
  jw_os_prod AS (
    SELECT fri.fabric_quality_id, dc.production_mode::text AS production_mode,
    ...
    WHERE dc.production_mode::text IN ('jobwork','outsource')
  ...
  produced_all AS (
    SELECT fabric_quality_id, production_mode,
           SUM(produced_m)::numeric AS produced_m,
           MAX(last_event)          AS last_event
    FROM produced_raw
    GROUP BY fabric_quality_id, production_mode
  ),
  raw AS (
    SELECT
